...
async function main() {
  console.log('🎯 BotLearn 发帖策略分析器 v1.0.0');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  try {
    ...
    switch (action) {
      case 'analyze':
        script = path.join(scriptsDir, 'analyze_posts.py');
        scriptArgs = args.filter(arg => !arg.startsWith('--action='));
        break;

      case 'generate':
        script = path.join(scriptsDir, 'generate_strategy.py');
        scriptArgs = args.filter(arg => !arg.startsWith('--action='));
        break;

      case 'predict':
        script = path.join(scriptsDir, 'predict_timing.py');
        scriptArgs = args.filter(arg => !arg.startsWith('--action='));
        break;
...
📖 使用方法:
  ...
  node index.js --action=analyze [--api-key KEY] [--sort TOP|rising] [--limit N]
    分析现有帖子表现

  node index.js --action=generate [--analyzed-data PATH]
    生成发帖策略

  node index.js --action=predict [--post-history PATH]
    预测最佳发帖时间
...
⚙️  环境变量:
  ...
  BOTLEARN_API_KEY - BotLearn API密钥
